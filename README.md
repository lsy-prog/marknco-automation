...
## ⚠️ 지금까지 결과물이 안 보였던 진짜 이유 → 고쳤습니다

Routine이 "완료했다"고 로그를 남겨도 매번 `output/`이 비어있던 진짜 원인은 권한 문제가
아니라 **Claude Code 세션이 실행마다 새로운 임시 브랜치(`claude/xxx-yyy-zzz` 형태)를 만들어
거기에만 push하고, `main`엔 한 번도 반영이 안 됐기 때문**이었습니다. GitHub 기본 화면은
`main`을 보여주니 아무리 확인해도 안 보였던 거예요.

**고친 내용** (`routine/ROUTINE_PROMPT.md`에 반영):
- 이제 Routine이 항상 `git checkout main → pull → 작업 → push origin main` 순서로
  **main에 직접** 커밋하도록 명시했습니다.
- push 후에는 반드시 GitHub에서 **main 브랜치 기준으로** 실제 반영 여부를 재확인하도록
  했습니다 (로그만 믿지 않도록).
- 이메일은 **PDF 첨부 없이, 완료 알림 + GitHub 링크**만 보내는 걸로 바꿨습니다. 연결된
  Gmail 커넥터에 애초에 "발송" 기능이 없는 경우도 있었고, 수 MB짜리 PDF를 첨부로 밀어넣는
  방식 자체가 이 구조에 안 맞았습니다.

**적용하려면 사용자님이 한 가지 해주셔야 해요**: Routine이 의존성 자동설치 인프라를 고쳐서
임시 브랜치에 올려둔 게 있을 텐데, GitHub 저장소에 떠 있는 **Pull Request를 main으로
머지**해주세요 (저장소 페이지 상단에 "Compare & pull request" 배너가 보일 거예요).
그래야 다음 실행부터 그 인프라 수정도 적용됩니다.

그 다음, Routine 프롬프트 칸의 내용을 이 저장소의 최신 `routine/ROUTINE_PROMPT.md`로
다시 교체해주세요 (프롬프트 텍스트는 저장소 파일을 자동으로 읽어오는 게 아니라, Routine
편집 화면에 직접 붙여넣어야 반영됩니다).
