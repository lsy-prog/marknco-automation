...
4. **결과 저장 및 전달**:
   - **반드시 `main` 브랜치에서 직접 작업할 것.** Claude Code 세션 기본 동작(세션마다 임시
     브랜치를 새로 만드는 것)을 따르지 말고, 명시적으로 `git checkout main && git pull origin main`
     으로 시작해서, 최종 커밋도 `git push origin main`으로 **main에 직접** 올린다.
     (임시 브랜치에 계속 올라가고 아무도 머지 안 해서 결과물이 영영 안 보이는 사고가
     실제로 여러 번 있었다 — 세션이 알아서 만드는 `claude/xxx-yyy-zzz` 브랜치를 절대 최종
     목적지로 쓰지 말 것.)
   - 최종 PDF(및 필요시 docx)를 `output/` 폴더에
     `KPATH_[롱리스트|지원기업]_YYYYMMDD_HHmm.pdf` 형식으로 저장하고 main에 커밋+푸시한다.
   - **push 성공 여부를 반드시 원격 저장소에서 직접 재확인할 것.** `git push`가 에러 없이
     끝났다는 로컬 출력만 믿지 말고, push 직후 GitHub API나 `git ls-remote origin main`으로
     **정확히 `main` 브랜치에** 방금 만든 커밋이 반영됐는지 확인한다 (다른 브랜치에 성공했다고
     착각한 사고, 존재하지 않는 브랜치에 성공했다고 잘못 보고한 사고 둘 다 있었다).
   - **이메일은 첨부파일 없이, 알림 형태로만 보낸다.** 수신자: `lsy@markncompany.co.kr`.
     제목: `[K-PATH 자동화] {프로그램명} 프로필 PDF 생성 완료 ({기업 수}개사, {날짜})`.
     본문: 처리한 기업 수, 페이지 수 분포, 스킵/제외된 기업과 사유, 그리고 결과 파일의
     **GitHub 링크**(`https://github.com/{owner}/{repo}/blob/main/output/파일명.pdf`).
     **PDF를 첨부하지 말 것** — 수 MB짜리 바이너리를 커넥터 도구 호출에 텍스트로 욱여넣는
     방식은 비현실적이라는 게 이미 확인됐다 (연결된 Gmail 커넥터에는애초에 "발송" 도구가
     없고 초안 생성만 가능한 경우도 있었다 — 이 경우 초안을 만들어두고 그 사실을 로그에
     남길 것. 발송 자체가 안 되면 실패로 처리하지 말고, 어차피 확정 경로는 GitHub main이므로
     이메일은 보조 알림으로만 취급한다).

5. **보고**: 어떤 스킬을 썼는지, 몇 개 기업을 처리했는지, 검증 결과(페이지 수, 표 정렬 오차),
   **main 브랜치에 실제로 반영됐는지 원격 확인 결과**, 이메일(또는 초안) 발송 시도 결과,
...
- **세션이 자동으로 만드는 임시 브랜치(`claude/xxx-yyy-zzz`)에 결과물을 남겨두고 끝내기** —
  반드시 `main`에 직접 push할 것
- **수 MB 이상 되는 PDF를 이메일 첨부/커넥터 도구 호출에 통째로 밀어넣기** — 대신 GitHub
  링크로 안내할 것
